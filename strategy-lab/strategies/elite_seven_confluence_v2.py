"""
Elite Seven Confluence v2 — Stricter confluence with macro trend filter.

Improvements over v1:
    - min_confluence raised to 5 (was 4) — only strongest signals
    - EMA200 trend filter: longs only above EMA200, shorts only below
    - SL tightened to 1.2 ATR (was 1.5) — less risk per trade
    - TP widened to 3.5 ATR (was 3.0) — better reward per trade
    - R:R improves from 2:1 to ~2.9:1

All 7 indicator modules unchanged — only the confluence threshold,
macro filter, and risk params are upgraded.
"""
...
class EliteSevenConfluenceV2:
    name = "elite_seven_confluence"
    version = "2.0"
    description = "7-indicator confluence — 5/7 threshold + EMA200 macro filter"

    params = {
        # -- Confluence & Risk ------------------
        "min_confluence": 5,          # was 4 — only the best setups
        "atr_period": 14,
        "atr_sl_mult": 1.2,          # was 1.5 — tighter stop
        "atr_tp_mult": 3.5,          # was 3.0 — wider target
        "ema200_period": 200,         # NEW — macro trend filter
    }

    # ---------------------------------------------------------------
    #  1. PENTARCH (identical to v1)
    # ---------------------------------------------------------------
    ...
    def _omnideck(self, df: pd.DataFrame) -> pd.Series:
        ...
        td9_bull = (buy_setup >= 9).astype(float) * 1.5
        td9_bear = (sell_setup >= 9).astype(float) * 1.5
    ...
    # ---------------------------------------------------------------
    #  3. VOLUME ORACLE (identical to v1)
    # ---------------------------------------------------------------
    ...
    def _augury_grid(self, df: pd.DataFrame) -> pd.Series:
        ...
        score_bull += trend_bull.astype(float) * 15
        score_bull += ema200_bull.astype(float) * 10
        score_bull += (adx_val > 20).astype(float) * 10
        score_bull += vol_spike.astype(float) * 10
        score_bull += rsi_ok_bull.astype(float) * 10
        score_bull += mom_bull.astype(float) * 10
        score_bull += hist_rising.astype(float) * 10
        score_bull += (~extension).astype(float) * 10
        score_bull += ((plus_di > minus_di) & (adx_val > 15)).astype(float) * 15
    ...
    # ---------------------------------------------------------------
    #  7. HARMONIC OSCILLATOR (identical to v1)
    # ---------------------------------------------------------------
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # ── EMA200 macro trend filter (NEW) ───────────────────
        ema200 = ema(close, p["ema200_period"])
        ...
        # Confluence signals with macro filter
        min_c = p["min_confluence"]
        df["signal"] = 0
        bull_only = (df["bull_votes"] >= min_c) & ~(df["bear_votes"] >= min_c) & macro_bull
        bear_only = (df["bear_votes"] >= min_c) & ~(df["bull_votes"] >= min_c) & macro_bear
...
Strategy = EliteSevenConfluenceV2
