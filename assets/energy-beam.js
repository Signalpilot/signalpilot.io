  ...
  function init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        // Volumetric fog/smoke - realistic layered clouds (ENHANCED)
        float fog(vec2 uv, float t, float beamDist) {
          float fogVal = 0.0;

          // Large billowing clouds - main volume (bigger, slower)
          vec2 p1 = uv * 0.8;
          p1.x += t * 0.025;
          p1.y += t * 0.015 + sin(t * 0.15) * 0.2;
          float cloud1 = noise(p1) * 0.5 + noise(p1 * 2.0 + t * 0.02) * 0.3 + noise(p1 * 4.0) * 0.2;
          cloud1 = smoothstep(0.25, 0.75, cloud1); // Softer cloud edges

          // Medium wisps - more prominent
          vec2 p2 = uv * 2.0;
          p2.x -= t * 0.035;
          p2.y += t * 0.02 + cos(t * 0.12) * 0.15;
          float cloud2 = noise(p2) * 0.5 + noise(p2 * 2.0 - t * 0.03) * 0.35 + noise(p2 * 3.5) * 0.15;
          cloud2 = smoothstep(0.3, 0.7, cloud2);

          // Fine detail layer
          vec2 p3 = uv * 3.5;
          p3.x += t * 0.04 + sin(t * 0.25) * 0.1;
          p3.y -= t * 0.025;
          float cloud3 = noise(p3) * 0.6 + noise(p3 * 1.8 + t * 0.04) * 0.4;
          cloud3 = smoothstep(0.35, 0.65, cloud3);

          // Swirling detail - more dramatic rotation
          vec2 p4 = uv * 5.0;
          float angle = t * 0.08;
          p4 = vec2(p4.x * cos(angle) - p4.y * sin(angle), p4.x * sin(angle) + p4.y * cos(angle));
          float cloud4 = noise(p4) * 0.7 + noise(p4 * 1.5) * 0.3;
          cloud4 = smoothstep(0.4, 0.6, cloud4);

          // Extra large background haze
          vec2 p5 = uv * 0.5;
          p5.x += t * 0.01;
          p5.y += sin(t * 0.1) * 0.1;
          float haze = noise(p5) * 0.6 + noise(p5 * 1.5 + t * 0.01) * 0.4;
          haze = smoothstep(0.2, 0.8, haze);

          // Combine layers with more weight (increased contributions)
          fogVal = cloud1 * 0.4 + cloud2 * 0.25 + cloud3 * 0.15 + cloud4 * 0.1 + haze * 0.1;

          // Spread fog more across the screen
          float rightBias = smoothstep(0.0, 0.5, (uv.x - 0.09)); // Fog spreads further right
          float beamFade = exp(-beamDist * 1.5) + rightBias * 0.6;
          beamFade = min(beamFade, 1.0);

          // Vertical distribution - more fog coverage
          float vertFog = smoothstep(0.0, 0.3, uv.y) * smoothstep(1.0, 0.4, uv.y);

          return fogVal * beamFade * vertFog * 1.2; // Increased intensity
        }
        ...
        void main() {
          ...
          // High-frequency noise for plasma/electrical effect (flows TOP to BOTTOM)
          ...
          // Flickering edge effect - chaotic boundary (flows downward)
          ...
          // Add rippling energy waves (flows downward)
          ...
          // Crackling effect at outer boundary (flows downward)
          ...
          // Tendril-like extensions (flows downward)
          ...
          // Energy flows from top to bottom
          ...
          // Volumetric fog/smoke - layered colors for depth (ENHANCED)
          vec3 fogColorDark = vec3(0.1, 0.08, 0.22);    // Deep dark base - slightly brighter
          vec3 fogColorMid = vec3(0.18, 0.14, 0.4);     // Mid violet - more prominent
          vec3 fogColorLight = vec3(0.3, 0.24, 0.55);   // Lighter highlights - more visible
          ...
          color += fogFinal * mysteriousFog * 2.2; // Increased intensity
