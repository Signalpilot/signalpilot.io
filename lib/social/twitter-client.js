...
async function searchTweets(query, limit = 10) {
  ...
  try {
    result = await tw.v2.search(query, {
    ...
    console.error('Twitter search exception:', JSON.stringify(errorInfo, null, 2));

    // If 429 (rate limit), return empty instead of throwing
    if (err.statusCode === 429) {
      console.warn(`Twitter rate limited for query: ${query}`);
      return { tweets: [], result_count: 0, rate_limited: true };
    }

    // If API key/auth issue, throw proper error
    if (err.statusCode === 401 || err.statusCode === 403) {
      throw new Error(`Twitter auth failed: ${err.message}`);
    }

    throw new Error(`Twitter API error: ${err.message || err.code || err.statusCode || 'Unknown exception'}`);
  }
  ...
  if (!result || !Array.isArray(result.data)) {
    if (result?.errors) {
      ...
      const errorMsg = errorDetail?.message || errorDetail?.title || JSON.stringify(errorDetail) || 'Unknown error';
      throw new Error(`Twitter API error: ${errorMsg}`);
    }
    if (result?.error) {
      throw new Error(`Twitter API error: ${result.error.message || result.error}`);
    }
