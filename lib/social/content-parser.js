...
function parsePart1Post(block) {
  ...
  return {
    postNumber,
    title,
    type: normalizeType(type) || inferTypeFromTitle(title),
    pillar,
    cta,
    source,
    twitter: { tweets },
...
function normalizeType(type) {
  ...
  const typeMap = {
    ...
    'education hub lesson (deep dive)': 'Education',
    'education': 'Education',
    'blog article': 'Blog',
    'blog': 'Blog',
    'quote card': 'Quote',
...
/**
 * Infer post type from title emojis and keywords (fallback when metadata missing)
 */
function inferTypeFromTitle(title) {
  if (!title) return '';
  // Emoji-based detection (Part 1 posts 61-325 use emojis in title)
  if (title.includes('\u{1F310}')) return 'Marketing';   // 🌐
  if (title.includes('\u{1F393}')) return 'Education';    // 🎓
  if (title.includes('\u{1F4DD}')) return 'Blog';         // 📝
  if (title.includes('\u{1F4AC}')) return 'Quote';        // 💬
  if (title.includes('\u{1F6E0}')) return 'Product';      // 🛠️
  if (title.includes('\u{1F52E}')) return 'Chronicle';    // 🔮
  if (title.includes('\u{1F4DA}')) return 'Docs';         // 📚
  // Keyword-based detection
  const upper = title.toUpperCase();
  // Marketing keywords
  if (upper.includes('MARKETING') || upper.includes('ROADMAP') || upper.includes('QUIZ')
    || upper.includes('SUCCESS STORY') || upper.includes('COMPARE VS') || upper.includes('FREE TRIAL')
    || upper.includes('MILESTONE') || upper.includes('COMPETITOR')) return 'Marketing';
  // Education keywords
  if (upper.includes('LESSON') || upper.includes('EDUCATION')) return 'Education';
  // Blog keywords
  if (upper.includes('BLOG') || upper.includes('(BLOG)')) return 'Blog';
  // Quote keywords
  if (upper.includes('QUOTE CARD') || upper.includes('QUOTE:')) return 'Quote';
  // Product keywords (indicator names, product features)
  if (upper.includes('DEMO') || upper.includes('INDICATOR') || upper.includes('PENTARCH')
    || upper.includes('AUGURY') || upper.includes('HARMONIC') || upper.includes('VOLUME ORACLE')
    || upper.includes('PLUTUS') || upper.includes('JANUS') || upper.includes('OMNIDECK')
    || upper.includes('DEEP DIVE') || upper.includes('SIGNAL DEEP') || upper.includes('SETUP')
    || upper.includes('(PRODUCT)')) return 'Product';
  // Chronicle keywords
  if (upper.includes('CHRONICLE') || upper.includes('LORE')) return 'Chronicle';
  // Docs keywords
  if (upper.includes('DOCS') || upper.includes('CHEATSHEET') || upper.includes('GUIDE')) return 'Docs';
  if (upper.includes('MANIFESTO') || upper.includes('LAUNCH')) return 'Manifesto';
  return '';
}
