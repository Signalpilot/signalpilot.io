 ...
 * Note: Requires instagram_business_discovery permission
...
async function searchPostsByHashtag(hashtag, limit = 10) {
  ...
  // Instagram doesn't have direct hashtag search in Graph API v21
  // Alternative: Use BUSINESS_DISCOVERY to find accounts, then get their media
  const url = `${graphBase}/${accountId}?fields=ig_hashtag_search&user_id=${accountId}&ig_hashtag_search(user_id=${accountId},hashtag_name=${hashtag})&access_token=${accessToken}`;

  const response = await fetch(url);
  const data = await response.json();

  if (data.error) {
    throw new Error(`Hashtag search failed: ${data.error.message}`);
  }
