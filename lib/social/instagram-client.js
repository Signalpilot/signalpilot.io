...
async function resolveAccountId() {
  ...
  if (tokenType === 'instagram') {
    ...
    // For EAA tokens (Facebook Login), accountId is a Facebook Page ID
    // We need to resolve it to the actual Instagram business account ID
    const url = `${FACEBOOK_GRAPH_BASE}/${accountId}?fields=instagram_business_account&access_token=${accessToken}`;
    const response = await fetch(url);
    const data = await response.json();
    if (data.error) {
      throw new Error(`Failed to resolve Instagram account from Facebook Page: ${data.error.message}`);
    }
    if (!data.instagram_business_account?.id) {
      throw new Error(`No Instagram business account linked to Facebook Page ${accountId}`);
    }
