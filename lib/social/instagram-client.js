...
async function searchPostsByHashtag(hashtag, limit = 10) {
  ...
  // Fallback: search popular trading accounts instead of hashtags
  const fallbackAccounts = ['investopedia', 'tradingview', 'seekingalpha', 'stockmarkettoday'];

  let allPosts = [];
  for (const account of fallbackAccounts) {
    try {
      const result = await getAccountPosts(account, 3);
      allPosts = allPosts.concat(result.posts || []);
    } catch (err) {
      // Log but continue with other accounts
      console.warn(`Failed to fetch posts from @${account}: ${err.message}`);
    }
  }

  if (allPosts.length === 0) {
    console.warn(`No posts found from fallback accounts for hashtag #${hashtag}`);
  }

  return {
    data: allPosts.slice(0, limit),
    message: `Using account-based discovery (hashtag #${hashtag} deprecated in API v21)`,
...
async function getAccountPosts(username, limit = 5) {
  ...
  // Single business_discovery call — includes media in one request
  const url = `${graphBase}/${accountId}?fields=business_discovery.username(${username}){id,name,username,media.limit(${limit}){id,caption,media_type,timestamp,like_count,comments_count}}&access_token=${accessToken}`;

  const response = await fetch(url);
  const data = await response.json();

  if (data.error) {
    throw new Error(`Account discovery failed for @${username}: ${data.error.message}`);
  }
  ...
  return {
    account_id: data.business_discovery.id,
