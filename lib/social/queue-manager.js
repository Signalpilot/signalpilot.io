...
const KEYS = {
  ...
  storiesLastPosted: 'social:stories:last_posted',
  storiesLastPostedAt: 'social:stories:last_posted_at',
  reelsLastPosted: 'social:reels:last_posted',
  reelsLastPostedAt: 'social:reels:last_posted_at',
...
// Platform → Redis key mapping (each platform gets its OWN counter)
const PLATFORM_KEYS = {
  twitter:   { posted: 'twitterLastPosted',   postedAt: 'twitterLastPostedAt' },
  instagram: { posted: 'instagramLastPosted', postedAt: 'instagramLastPostedAt' },
  stories:   { posted: 'storiesLastPosted',   postedAt: 'storiesLastPostedAt' },
  reels:     { posted: 'reelsLastPosted',     postedAt: 'reelsLastPostedAt' },
};

async function getLastPosted(platform) {
  const kv = getKV();
  const mapping = PLATFORM_KEYS[platform] || PLATFORM_KEYS.instagram;
  const key = KEYS[mapping.posted];
  const timeKey = KEYS[mapping.postedAt];
...
async function getStatus() {
  ...
  const stories = await getLastPosted('stories');
  const reels = await getLastPosted('reels');
  ...
  return {
    ...
    stories: {
      lastStoryCounter: stories.postOrder,
      lastPostedAt: stories.postedAt,
      nextStoryCounter: stories.postOrder + 1,
    },
    reels: {
      lastPostOrder: reels.postOrder,
      lastPostedAt: reels.postedAt,
      nextPostOrder: reels.postOrder + 1,
    },
    token: {
