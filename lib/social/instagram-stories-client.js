...
async function resolveAccountId() {
  ...
  if (tokenType === 'instagram') {
    // IGAAM tokens: query /me on graph.instagram.com to get real IG user ID
    ...
    // EAA tokens: resolve Instagram Business Account from Facebook Page ID
    if (directAccountId) {
      ...
      const data = await response.json();
      if (data.error) {
        console.warn(`Failed to resolve IG account from Page ${accountId}: ${data.error.message}. Using accountId directly.`);
  ...
  console.log(`Resolved account ID: ${_resolvedAccountId} (token type: ${tokenType})`);
